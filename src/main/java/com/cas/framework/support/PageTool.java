...
import com.cas.framework.utils.UrlUtil;
...
public class PageTool implements Serializable {
	...
	public PageTool(int currentPage, int pageSize, int size, Page<?> page) {
		if (size <= 0) {
			this.currentPage = 1;
			this.startPage = 1;
			this.endPage = 1;
	...
	public String getNewHref() {
		String str = "";
		if (StringUtils.isNotEmpty(getHref())) {
			if (getHref().indexOf("?") > 0) {//如果有‘?’
				str += getHref() + "&";
			} else {
				str += getHref() + "?";
			}
